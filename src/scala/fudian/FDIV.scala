...
package connx_npu.fudian

import chisel3._
import chisel3.util._
import connx_npu.fudian.fudian._
import connx_npu.fudian.utils.{CLZ, LZA}
...
class DivIterModule(len: Int, itn_len: Int) extends Module {
  ...
  // rudNeg := VecInit(Seq.tabulate(5){i => udNeg(i)(itn_len-2, itn_len-5)})
  // r2udNeg := VecInit(Seq.tabulate(5){i => udNeg(i)(itn_len-2, itn_len-7)})
  ...
  // r3ws := wsReg(itn_len-1, itn_len-7)
  // r3wc := wcReg(itn_len-1, itn_len-7)

  r2ws := wsReg(itn_len-1, itn_len-10)
  r2wc := wcReg(itn_len-1, itn_len-10)
  // r2ws := wsReg(itn_len-1, itn_len-4)
  // r2wc := wcReg(itn_len-1, itn_len-4)
  ...
  qSpec := VecInit(Seq.tabulate(5){ q_spec => {
    ...
    // csa1.io.in(2) := SignExt(udNegReg(q_spec)(itn_len-2, itn_len-5), 13) // (4, 6) -> (7, 6)
