...
import connx_npu.fudian.utils._
...
class FADD(val expWidth: Int, val precision: Int) extends Module {
  ...
  val FCMA_ADDdef = Module(new FCMA_ADD(expWidth, precision, precision))

  FCMA_ADDdef.io.a := io.a
  FCMA_ADDdef.io.b := io.b
  FCMA_ADDdef.io.rm := io.rm
  FCMA_ADDdef.io.b_inter_valid := false.B
  FCMA_ADDdef.io.b_inter_flags := DontCare
  io.result := FCMA_ADDdef.io.result
  io.fflags := FCMA_ADDdef.io.fflags
}
