### script
def init(host):
    host.data[0x200000] = jnp.array([v * 1.1 + 0.01 for v in range(512)], dtype=jnp.bfloat16).tobytes()
    host.data[0x200400] = jnp.array([v * 0.1 + 0.01 for v in range(512)], dtype=jnp.bfloat16).tobytes()
...
def finalize(host):
    ...
    A = jnp.array([v * 1.1 + 0.01 for v in range(512)], dtype=jnp.bfloat16)
    B = jnp.array([v * 0.1 + 0.01 for v in range(512)], dtype=jnp.bfloat16)
